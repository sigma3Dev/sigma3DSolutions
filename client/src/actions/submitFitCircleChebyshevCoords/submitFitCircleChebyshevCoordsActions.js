export const SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS = 'SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS';
export const SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_REQUEST =
  'SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_REQUEST';
export const SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_SUCCESS =
  'SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_SUCCESS';
export const SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_FAILURE =
  'SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_FAILURE';

const axios = require('axios');

/** doesn't do anything, fires whenever submitFitCircleChebyshevCoords is requested */
export const submitFitCircleChebyshevCoordsRequest = () => ({
  type: SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_REQUEST,
  receivedAt: Date.now(),
});

/**
 * fired when a response is returned from the backend
 * @param {Object} response - Response that is returned from the node server
 */
export const submitFitCircleChebyshevCoordsSuccess = response => ({
  type: SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_SUCCESS,
  response,
  receivedAt: Date.now(),
});

/**
 * fired when an error is returned from the backend
 * @param {Object} error - Error that is returned from the node server
 */
export const submitFitCircleChebyshevCoordsFailure = error => ({
  type: SUBMIT_FIT_CIRCLE_CHEBYSHEV_COORDS_FAILURE,
  error,
  receivedAt: Date.now(),
});

/**
 * sends an axios request to the backend
 * @param {dispatchCallback} dispatch - callback that handles dispatching of the action
 * @param {getStateCallback} getState - callback that gets current state
 * @returns {Object} response from backend or error message
 */
export const submitFitCircleChebyshevCoords = coords => (dispatch) => {
  dispatch(submitFitCircleChebyshevCoordsRequest());
  return axios
    .post('/fit-circle-chebyshev', {
      coords,
    })
    .then((response) => {
      if (Object.prototype.hasOwnProperty.call(response.data, 'result')) {
        dispatch(submitFitCircleChebyshevCoordsSuccess(response));
      } else {
        dispatch(submitFitCircleChebyshevCoordsFailure(response.data.error.message));
      }
    })
    .catch((error) => {
      dispatch(submitFitCircleChebyshevCoordsFailure(error.message));
    });
};
